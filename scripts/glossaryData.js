...
window.GLOSSARY = {
  ...
  "agent": {
    title: "LLM agent",
    summary: "A language model wrapped in a loop that observes, plans, calls tools, and acts on an external environment until a goal is met. The defining shift from chatbot to agent is the closed loop with the world: the model decides what to do next based on what just happened, so a one-shot text predictor becomes a system that takes steps over time.",
    resources: [
  ...
  "aime": {
    ...
    summary: "A 15-problem, 3-hour high-school competition math test with integer answers from 0 to 999, used as a hard reasoning benchmark for LLMs. Because answers are short integers, AIME is easy to grade automatically, which is why it became a favorite for verifiable-reward RL setups. DeepSeek-R1 and OpenAI's o-series report headline numbers as <em>AIME 2024 pass@1</em>.",
    resources: [
  ...
  "autogpt": {
    title: "Auto-GPT",
    summary: "A March 2023 open-source experiment that wrapped GPT-4 in a self-prompting loop: given a high-level goal, it generates subtasks, executes them via tool calls (web browse, file I/O, shell), critiques its own progress, and continues until the goal is met. It went viral as a proof-of-concept of autonomous agents and was widely unreliable in practice. It shipped the idea of agents before the underlying models could sustain the loop.",
    resources: [
  ...
  "bradley-terry": {
    ...
    summary: "A probability model (1952) for pairwise comparisons: assign each item a positive score, and the probability that <em>i</em> beats <em>j</em> is its share of their combined score. Fitting the scores by maximum likelihood from observed comparisons gives a ranking. RLHF's reward model is typically trained with a Bradley-Terry loss on human preference pairs.",
    resources: [
  ...
  "budget-forcing": {
    ...
    summary: "A test-time-compute trick from s1 (Muennighoff et al., 2025): force the model to keep thinking by appending the token <code>Wait</code> whenever it tries to stop, or force it to stop by inserting an end-of-thinking marker. The result is a knob on how many reasoning tokens the model produces, trading compute for accuracy without retraining.",
    resources: [
  ...
  "chain-of-thought": {
    ...
    summary: "Prompting a large model to produce intermediate reasoning steps before its final answer, often by adding a cue like <em>let's think step by step</em>. The extra tokens give the model room to compute, so accuracy on multi-step problems jumps without any weight change. It was the first widely cited proof that how a prompt is phrased is worth measurable accuracy.",
    resources: [
  ...
  "codeforces-elo": {
    ...
    summary: "The Elo-style competitive-programming rating from codeforces.com, increasingly used as a yardstick for code-reasoning models. OpenAI's o-series and DeepSeek-R1 report Codeforces percentile and rating numbers head-to-head with human contestants. It is attractive because problems are graded by hidden tests, making the score hard to game.",
    resources: [
  ...
  "computer-use": {
    title: "Computer use",
    summary: "An agent capability in which the model directly drives a desktop or browser by observing screenshots and emitting mouse, keyboard, and scroll actions, using the same interface a human uses. Anthropic shipped the first general-purpose computer-use API with Claude 3.5 Sonnet (October 2024); OSWorld is the canonical benchmark.",
    resources: [
  ...
  "deliberative-alignment": {
    ...
    summary: "OpenAI's training recipe for the o-series (Guan et al., Dec 2024): teach the model to explicitly reason over a written safety spec in its own chain-of-thought before answering, rather than to imitate safe answers directly. Applied to the o-series using only synthetic data (no human-written CoT, no human-written answers), it reports better jailbreak resistance, less over-refusal, and out-of-distribution generalization to safety cases not in the training set. Turns the Model Spec from a filter into a document the model was taught to think against.",
    resources: [
  ...
  "discrete-prompt-optimization": {
    ...
    summary: "Searching directly over <em>words</em> (the discrete vocabulary) rather than over continuous embeddings: propose a candidate instruction, score it on a small dev set, keep the winners, mutate, repeat. APE, OPRO, EvoPrompt, PromptBreeder, and GEPA all live in this family. The appeal is that the resulting prompt is human-readable and portable across models.",
    resources: [
  ...
  "double-quantization": {
    ...
    summary: "A QLoRA trick: after quantizing weights to 4-bit NF4, also quantize the per-block quantization constants themselves (from FP32 down to 8-bit) so the metadata overhead drops by another ~0.37 bits per parameter. Small but real, and part of why QLoRA can finetune a 65B model on a single 48GB GPU.",
    resources: [
  ...
  "emergent-ability": {
    ...
    summary: "A capability that is essentially absent at small scale but appears past some compute, parameter, or data threshold. Wei et al. (2022) catalogued dozens of such jumps in LLMs. A 2023 NeurIPS paper (Schaeffer et al.) argued that many such emergence claims are artifacts of harsh metrics and disappear under continuous scoring. The debate is unsettled.",
    resources: [
  ...
  "encoder-only": {
    ...
    summary: "A transformer that uses only the bidirectional encoder stack (every token attends to every other token) and is typically trained with masked language modeling. BERT (Devlin et al., 2018) was the prototype. Encoder-only models do well at understanding tasks like classification and retrieval, but cannot generate text autoregressively.",
    resources: [
  ...
  "gepa": {
    ...
    summary: "A reflective prompt optimizer (Agrawal et al., 2025): after each attempt it writes a natural-language diagnosis of why the output fell short, proposes a targeted edit aimed at exactly that failure, and keeps a Pareto frontier of candidates so it never collapses to one brittle winner. Matched or beat GRPO by up to ~20 points using up to 35x fewer rollouts; ICLR 2026 oral.",
    resources: [
  ...
  "icl": {
    ...
    summary: "A frozen language model performs a brand-new task purely from examples or instructions placed in its prompt, with no gradient updates. Few-shot uses a handful of examples; zero-shot uses just an instruction. GPT-3 was the first widely noticed demonstration, and it is why a prompt is a steering lever rather than only a question.",
    resources: [
      { label: "GPT-3 paper (Brown et al., 2020)", url: "https://arxiv.org/abs/2005.14165" },
  ...
  "intent-alignment-gap": {
    ...
    summary: "The gap between what the user wanted and what the model optimizes for. A base LM optimized for next-token likelihood is not optimized to be helpful, honest, or harmless; InstructGPT framed RLHF specifically as a way to close this gap by training on human preferences over assistant behavior.",
    resources: [
      { label: "Ouyang et al., 2022: InstructGPT", url: "https://arxiv.org/abs/2203.02155" },
  ...
  "ipo": {
    ...
    summary: "Azar et al., Oct 2023. A DPO variant that replaces DPO's Bradley-Terry log-sigmoid with an identity mapping, so the loss stops running off to infinity on near-certain preference pairs. Same offline setup as DPO with no separate reward model, less prone to overfitting on confident preference labels.",
    resources: [
  ...
  "kl-penalty": {
    title: "KL penalty",
    summary: "The same idea as the KL constraint, expressed as a soft penalty term in the reward: <code>r = r_RM &minus; &beta; &middot; KL(&pi; || &pi;_ref)</code>. The hyperparameter &beta; trades off reward-chasing against staying close to the reference SFT policy. Tuning &beta; is one of the dark arts of RLHF.",
    resources: [
  ...
  "llama": {
    title: "LLaMA",
    summary: "Meta's open-weights LLM family. LLaMA 1 (Feb 2023, Touvron et al.) showed that a 13B model trained on 1T tokens could match GPT-3 175B, validating Chinchilla's data-heavy regime. LLaMA 2, 3, and 3.1 made permissively licensed open weights the substrate that the entire open-source post-training stack (Alpaca, Vicuna, QLoRA, R1) built on.",
    resources: [
  ...
  "mtp": {
    ...
    summary: "Train the model with several shallow prediction heads that guess the next D tokens in parallel (each conditioned on the previous head's guess), instead of only the next token. Popularized by DeepSeek-V3, which reports a quality gain from the denser gradient signal. The extra heads also double as draft-token producers for speculative decoding at inference time, giving an MTP-trained model a serving speedup for free.",
    resources: [
      { label: "DeepSeek-V3 technical report", url: "https://arxiv.org/abs/2412.19437" },
  ...
  "next-token-prediction": {
    ...
    summary: "The training objective of decoder-only LLMs: given the prefix, predict the next token, and do that for trillions of tokens. The claim of the scale era is that this single objective, run on enough data with enough parameters, is a universal training signal that yields reasoning, world knowledge, and instruction-following as side effects.",
    resources: [
      { label: "Radford et al., 2018: GPT", url: "https://cdn.openai.com/research-covers/language-unsupervised/language_understanding_paper.pdf" },
  ...
  "opro": {
    ...
    summary: "A 2023 DeepMind method (Yang et al.) that uses an LLM itself as the optimizer: at each step it sees a history of past prompts and their scores, and proposes a new prompt aimed at higher score. OPRO showed that an LLM can do non-trivial black-box optimization purely through prompting, and discovered prompts that beat <em>let's think step by step</em> on GSM8K.",
    resources: [
  ...
  "peft": {
    ...
    summary: "The umbrella term for fine-tuning methods that freeze most of the pretrained model and update only a small set of new or selected parameters: LoRA, prefix tuning, prompt tuning, adapters, (IA)&sup3;, and so on. PEFT methods can match full fine-tuning quality while updating &lt;1% of weights, cutting memory and storage costs per task.",
    resources: [
  ...
  "power-law": {
    ...
    summary: "A relationship of the form <code>y = a &middot; x^b</code>, which is a straight line on a log-log plot. LLM loss decreases as a power law in parameters, data, and compute (Kaplan et al. 2020, Hoffmann et al. 2022). The smooth, predictable shape of these curves is what made <em>just scale it up</em> a viable strategy in 2020-2022.",
    resources: [
  ...
  "process-reward-model": {
    ...
    summary: "A reward model trained to score each <em>step</em> of a chain-of-thought, not only the final answer, so the RL signal can credit good intermediate reasoning even when the final answer is wrong. Lightman et al. (2023) released PRM800K, 800K step-level human-labeled annotations of MATH solutions, which trained the first widely cited PRM.",
    resources: [
  ...
  "process-vs-outcome-reward": {
    ...
    summary: "Two ways to give RL feedback on reasoning. <em>Outcome reward</em> only checks the final answer (cheap, easy to verify, but blind to how the model got there). <em>Process reward</em> scores each step (denser signal, fewer credit-assignment problems, but expensive to label). DeepSeek-R1 used pure outcome reward (RLVR) and still produced sophisticated reasoning traces.",
    resources: [
  ...
  "promptbreeder": {
    title: "PromptBreeder",
    summary: "A 2023 DeepMind method (Fernando et al.) that evolves both prompts and the mutation operators used to evolve them: a self-referential evolutionary loop where the LLM both produces candidates and proposes how to mutate them. It outperformed CoT, APE, and OPRO on GSM8K and BBH at the time.",
    resources: [
  ...
  "rlvr": {
    ...
    summary: "Use a programmatic verifier (a unit test, an equation checker, an integer-answer comparator) as the reward function, with no learned reward model needed. Coined in the Tulu 3 paper (Nov 2024) and popularized by DeepSeek-R1. Verifiable rewards are harder to hack per-sample than learned RMs, so they enable much longer, more aggressive RL training. They are not un-hackable: length bias, format hacking, and spec exploits are all real, and Ch 7 covers the failure modes and successor algorithms in detail.",
    resources: [
  ...
  "sharegpt": {
    title: "ShareGPT",
    summary: "A community dataset of ~90K real ChatGPT conversations scraped from the now-defunct sharegpt.com extension, used to fine-tune Vicuna and many other early open chat models. Its existence fueled the 2023 explosion of GPT-3.5-distilled open assistants, with all the licensing and provenance ambiguity that implies.",
    resources: [
  ...
  "speculative-decoding": {
    ...
    summary: "A lossless inference-time speedup: a small draft model proposes the next k tokens, the big target model verifies all k in a single parallel forward pass, and every drafted token that matches what the target would have picked is a token accepted for free. Introduced independently by Leviathan et al. (Google, 2022) and Chen et al. (DeepMind, 2023). Because decode is memory-bandwidth-bound, verifying k+1 positions costs about the same wall-clock as verifying one, so the trick is a step-function speedup for the target's own arithmetic. Modern drafters (EAGLE, Medusa, MTP heads) reuse the target's hidden states to raise acceptance rates.",
    resources: [
  ...
  "vicuna": {
    title: "Vicuna",
    summary: "An LMSYS project (March 2023) that fine-tuned LLaMA-13B on 70K ShareGPT conversations and reported ~90% GPT-4-judged quality vs. ChatGPT for ~$300 of compute. Vicuna kicked off the LLM-as-judge era (its eval framework became MT-Bench and then Chatbot Arena) and was the most influential open chat model of mid-2023.",
    resources: [
